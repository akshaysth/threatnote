    ...
    for url in urls:
        ...
        try:
            response = requests.post(
                url, json.dumps(wh_data), headers={"Content-Type": "application/json"}
            )
            if response.status_code != 200:
                print(
                    'Webhook error sending message "{}" to url {}. Response code: {} Response text: {}'.format(
                        wh_data, url, response.status_code, response.text
                    )
                )
            else:
            ...
            print(
                'Webhook error sending message "{}" to url {}. {}'.format(
                    wh_data, url, err
                )
            )
...
def get_comments(req_id=None, report_id=None, indicator_id=None):
    ...
    if report_id:
        comments = (
            db.session.query(Comments, User.name)
...
def link_exists(new_link):
    link = (
        Links.query.filter_by(indicator=new_link.indicator)
        .filter_by(report=new_link.report)
        .first()
    )
...
def parse_indicators(summary, report_id, queue):
    ...
    for match in attacks:
        ...
        if match_check:
            ...
            new_indicator = Indicators(
                indicator=match, indicator_type="MITRE ATT&CK Technique"
            )
            add_db_entry(new_indicator)
            indicator_ids[new_indicator.id] = match
    ...
    for id in indicator_ids:
        ...
        job = queue.enqueue(
            main_bp.enrich_pipeline,
            json.dumps({"indicator": str(match), "organization": org_id}),
        )
        new_link = Links(
            indicator=id,
            report=report_id,
            kill_chain="Unknown",
            diamond_model="Unknown",
            confidence="Low",
        )
