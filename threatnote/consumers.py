...
@consumers_bp.route("/consumers", methods=["GET"])
@login_required
def view_consumers():
    consumer_dict = {}
    ...
    ir_counts = (
        db.session.query(
            RequirementConsumers.consumer,
            func.count(RequirementConsumers.requirement),
            func.min(Requirements.created_at),
            func.max(Requirements.updated_at),
        )
    ...
    report_counts = (
        db.session.query(
            RequirementConsumers.consumer,
            func.count(RequirementReports.report),
            func.min(Reports.created_at),
            func.max(Reports.updated_at),
        )
        .join(
            RequirementReports,
    ...
    return render_template(
        "consumers.html", consumers=consumer_dict.values(), page_title="Consumers"
    )


@consumers_bp.route("/edit_consumer/<consumer_id>")
@login_required
...
@consumers_bp.route("/edit_consumer/<consumer_id>", methods=["POST"])
@login_required
def update_consumer(consumer_id):
    ...
    poc = args.get("poc")
    Consumers.query.filter_by(id=consumer_id).update(
        {"subtitle": subtitle, "email": email, "poc": poc}
    )
    db.session.commit()
    db.session.flush()
