...
@reqs_bp.route("/active_reqs")
@reqs_bp.route("/active_reqs/<consumer_id>")
...
def list_reqs(consumer_id=None):
    ...
    if show_archived:
        if consumer_id:
            ...
            active_reqs = (
                ...
                .join(
                    RequirementConsumers,
                    Requirements.id == RequirementConsumers.requirement,
                )
        ...
        if consumer_id:
            ...
            active_reqs = Requirements.query.filter(
                Requirements.is_archived.is_(False)
            ).all()
    ...
    return render_template(
        ...
        page_title="Intel Requirements",
        show_archived=show_archived,
    )


@reqs_bp.route("/archive_req/<req_id>", methods=["GET", "POST"])
@login_required
...
@reqs_bp.route("/delete_req/<req_id>")
@login_required
...
def edit_req(req_id):
    ...
    consumers = (
        ...
        .join(RequirementConsumers, RequirementConsumers.consumer == Consumers.id)
        .filter(RequirementConsumers.requirement == req_id)
        .all()
    )
    ...
    return render_template(
        "edit_req.html",
        req_info=req_info,
        final_consumers=final_consumers,
        page_title=req_info.title,
    )


@reqs_bp.route("/new_ir")
@login_required
def new_ir():
...
@reqs_bp.route("/requirements/friendly_id_check/")
@login_required
...
@reqs_bp.route("/submit_ir", methods=["POST"])
@login_required
def submit_ir():
    form_data = request.form
    #    created_at = datetime.now().strftime('%m-%d-%y %H:%M:%S')
    try:
        time_requirement = datetime.strptime(
            form_data.get("time_requirement"), "%m/%d/%Y"
        ).date()
    ...
    if form_data.get("is_edit") == "True":
        ...
        hooks = Organization.query.filter(
            Organization.slack_webhook_on_req_update.is_(True)
        ).all()
            ...
            "attachments": [
                {
                    ...
                    "color": "#6658EA",
                    "fields": [
                        {"title": "Writer", "value": req.creator, "short": "true"},
                        {
                            "title": "Due Date",
                            "value": req.time_requirement.strftime(
                                "%m-%d-%y"
                            ),  # gotta convert to string
                            "short": "true",
                        },
                        {"title": "ID", "value": req.friendly_id, "short": "true"},
        ...
        req = Requirements(
            ...
            friendly_id=form_data.get("friendly_id"),
            collection_requirements=form_data.get("collection_requirements"),
        ...
        # send a webhook
        hooks = Organization.query.filter(
            Organization.slack_webhook_on_req_create.is_(True)
        ).all()
        hooks = [hook.slack_webhook for hook in hooks if hook.slack_webhook]
    ...
    org_id = User.query.filter_by(id=current_user.id).first().organization
    add_requirement_consumer_links(
...
def view_req(req_id):
    ...
    linked_reports = (
        db.session.query(Reports, RequirementReports)
        .join(RequirementReports, Reports.id == RequirementReports.report)
        .filter(RequirementReports.requirement == req_id)
        .all()
    )
    ...
    if request.args.get("pdf"):
        html = render_template(
            ...
            final_consumers=final_consumers,
            comments=get_comments(req_id=req_id),
            user_id=current_user.id,
            ir_date=datetime.now(),
        )
...
def delete_requirement_consumer_links(req_id=None, consumer_id=None):
    if req_id:
        RequirementConsumers.query.filter(
            RequirementConsumers.requirement == req_id
        ).delete()
        db.session.commit()
    if consumer_id:
        RequirementConsumers.query.filter(
            RequirementConsumers.consumer == consumer_id
        ).delete()
        db.session.commit()
