...
from threatnote.models import (
    Links,
    Reports,
    Organization,
    Requirements,
    RequirementReports,
    RequirementConsumers,
    Indicators,
    ReportTags,
    User,
    Organization,
...
from threatnote.main import queue
...
@reports_bp.route("/active_reports")
@reports_bp.route("/active_reports/<consumer_id>")
@login_required
def active_reports(consumer_id=None):
    return list_reports(consumer_id)


@reports_bp.route("/list_reports")
@reports_bp.route("/list_reports/<consumer_id>")
@login_required
    ...
    if show_archived:
        if consumer_id:
            reports = (
                ...
                .join(
                    RequirementConsumers,
                    RequirementConsumers.requirement == RequirementReports.requirement,
                )
            ...
            reports = (
                ...
                .filter(
                    func.lower(ReportTags.tag) == func.lower(request.args.get("tag"))
                )
                .all()
            )
        else:
...
@reports_bp.route("/archive_report/<report_id>", methods=["GET", "POST"])
@login_required
def archive_report(report_id):
    ...
    return redirect(
        url_for(request.args.get("landing", "edit_report"), report_id=report_id)
    )


@reports_bp.route("/unarchive_report/<report_id>", methods=["GET", "POST"])
@login_required
...
# @reports_bp.route('/archived_reqs')


@reports_bp.route("/delete_report/<report_id>")
@login_required
...
@reports_bp.route("/edit_report/<report_id>")
@login_required
def edit_report(report_id):
    ...
    all_reqs = (
        db.session.query(Requirements).filter(Requirements.is_archived.is_(False)).all()
    )

    linked_requirements = (
        db.session.query(Requirements)
        .join(RequirementReports, RequirementReports.requirement == Requirements.id)
        .filter(RequirementReports.report == report_id)
        .all()
    )
...
@reports_bp.route("/new_report")
@login_required
def new_report():
    all_reqs = (
        db.session.query(Requirements.id, Requirements.friendly_id, Requirements.title)
...
def submit_report():
    ...
    if request.form.get("is_edited"):
        ...
        # send a webhook
        hooks = Organization.query.filter(
            Organization.slack_webhook_on_report_create.is_(True)
        ).all()
        hooks = [hook.slack_webhook for hook in hooks if hook.slack_webhook]

        message = "A new report, {},  has been created. To view the report, go to: {}{}".format(
            report.title,
            request.host_url[0:-1],
            url_for("view_report", report_id=report.id),
        )
        wh_data = {
            "attachments": [
                {
                    ...
                    "color": "#6658EA",
                    "fields": [
                        {"title": "Writer", "value": report.creator, "short": "true"},
                        {"title": "ID", "value": report.friendly_id, "short": "true"},
                        {
                            "title": "Title",
                            "value": report.title,
                        },
...
def view_report(report_id):
    ...
    indicators = (
        db.session.query(Indicators)
        .join(Links, Indicators.id == Links.indicator)
        .join(Reports, Links.report == Reports.id)
    ...
    for indicator in indicator_dict.values():
        if (
            ...
            indicator_counts[indicator["indicator"]] = (
                indicator_counts.get(indicator["indicator"], 0) + 1
            )
            ...
            if regexp.search(word):
                ...
                if is_code:
                    ...
                    if search_word in ip_list:
                        new_content = (
                            '{}<span class="{}" id="{}{}">{}</span>{} '.format(
                                new_content,
                                ip_class,
                                escape_jquery(txt),
                                indicator_counts.get(txt, 0),
                                search_word,
                                delim,
                            )
                        )
                        ...
                        search_word in domain_list and word not in email_list + ip_list
                    ):
                        new_content = (
                            '{}<span class="{}" id="{}{}">{}</span>{} '.format(
                                new_content,
    ...
    links = (
        ...
        .join(Links, Reports.id == Links.report)
        .filter(Links.report != report_id)
        .filter(Links.indicator.in_(indicator_dict.keys()))
        .all()
    )
    for (report, link) in links:
        ...
        if indicator_record:
            ...
            if report.title != "" and report.id != int(report_id):
                ...
                report_list[report.id] = {
                    "id": report.id,
                    "title": report.title,
                    "friendly_id": report.friendly_id,
                }
    ...
    if request.args.get("pdf"):
        ...
        html = render_template(
            "blog-post-cover.html",
            report_info=report_info,
            indicator_counts=indicator_counts,
            new_content=content,
            indicators=indicator_dict.values(),
...
def delete_report_requirement_links(report_id=None, req_id=None):
    if report_id:
        ...
        db.session.commit()
    if req_id:
        RequirementReports.query.filter(
            RequirementReports.requirement == req_id
        ).delete()
        db.session.commit()
    ...
    for matchedtext in re.findall(reg, content):
        if len(matchedtext) > 1:
            if matchedtext[1]:
                ...
                rep = matchedtext[0]
            content = content.replace(
                "[{}]({})".format(matchedtext[0], matchedtext[1]), rep
            )
    markdown_re = [
        [r"(?<=\*\*).*?(?=\*\*)", "**{}**"],  # bold markdown
...
@reports_bp.route("/report/friendly_id_check/")
@login_required
