...
def homepage():
    ...
    consumer_dict = {}
    consumers_for_user = (
        db.session.query(Consumers)
        .join(User, User.organization == Consumers.organization)
    ...
    ir_counts = (
        db.session.query(
            RequirementConsumers.consumer, func.count(RequirementConsumers.requirement)
        )
    ...
    report_counts = (
        db.session.query(
            RequirementConsumers.consumer, func.count(RequirementReports.report)
        )
        .join(
            RequirementReports,
            RequirementReports.requirement == RequirementConsumers.requirement,
        )
        .filter(RequirementConsumers.consumer.in_(consumer_dict.keys()))
        .group_by(RequirementConsumers.consumer)
        .all()
    ...
    for id in consumer_dict:
        customer_list.append(
            {
                "customer": consumer_dict[id],
                "reports": report_counts_by_consumer.get(id, 0),
                "intelreqs": intel_counts_by_consumer.get(id, 0),
            }
        )
    ...
    domains = (
        ...
        .filter(Indicators.indicator_type == "Domain")
        .count()
    )
    urls = (
        db.session.query(Indicators).filter(Indicators.indicator_type == "URL").count()
    )
    emails = (
        db.session.query(Indicators)
        .filter(Indicators.indicator_type == "Email")
        .count()
    )
    sha256 = (
        db.session.query(Indicators)
        .filter(Indicators.indicator_type == "SHA256 Hash")
        .count()
    )
    md5 = (
        db.session.query(Indicators)
        .filter(Indicators.indicator_type == "MD5 Hash")
        .count()
    )
    cve = (
        db.session.query(Indicators).filter(Indicators.indicator_type == "CVE").count()
    )
    attack = (
        db.session.query(Indicators)
        .filter(Indicators.indicator_type == "MITRE ATT&CK Technique")
        .count()
    )
    ...
    active_reqs = (
        Requirements.query.filter(Requirements.is_archived.is_(False))
        .order_by(asc(Requirements.time_requirement))
        .limit(3)
        .all()
    )
    ...
    req_consumers = (
        db.session.query(Consumers, RequirementConsumers)
    ...
    if len(active_reqs) > 0:
        ...
        first_content = {}
        if days < 0:
            first_content["due_date"] = "This requirement was due %s days ago." % (
                str(str(days).split("-")[1])
            )
        elif days == 0:
            first_content["due_date"] = "This requirement is due today."
        else:
            first_content["due_date"] = "This requirement is due in %s days." % (
                str(days)
            )
    ...
    if len(active_reqs) > 1:
        ...
        second_content = {}
        if days < 0:
            second_content["due_date"] = "This requirement was due %s days ago." % (
                str(str(days).split("-")[1])
            )
        elif days == 0:
            second_content["due_date"] = "This requirement is due today."
        else:
            second_content["due_date"] = "This requirement is due in %s days." % (
                str(days)
            )
    ...
    if len(active_reqs) > 2:
        ...
        third_content = {}
        if days < 0:
            third_content["due_date"] = "This requirement was due %s days ago." % (
                str(str(days).split("-")[1])
            )
        else:
            third_content["due_date"] = "This requirement is due in %s days." % (
                str(days)
            )
...
@main_bp.route("/export_misp/<report_id>", methods=["GET", "POST"])
@login_required
def export_misp(report_id):
    report = Reports.query.filter_by(id=report_id).first()
    user_details = get_user_info(current_user.id)
    indicators = (
        db.session.query(Indicators.id, Indicators.indicator, Indicators.indicator_type)
...
def custom_search():
    ...
    reports_result = Reports.query.filter(
        Reports.content.like("%" + query_string + "%")
    )
    reports_result = reports_result.order_by(Reports.id).all()

    req_result = Requirements.query.filter(
        Requirements.summary.like("%" + query_string + "%")
    )
...
@main_bp.route("/comment/add", methods=["POST"])
@login_required
def add_comment(obj_type=None, obj_id=None, user=None, comment=None):
    ...
    if (
        ...
        and user
        and comment
        and obj_type in ("report", "requirement", "req", "indicator")
    ):
