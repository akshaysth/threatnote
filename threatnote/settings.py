...
@settings_bp.route("/user_webhooks", methods=["GET", "POST"])
@login_required
def webhooks():
    if current_user.role == "admin":
        ...
        if request.method == "POST":
            ...
            org.slack_webhook_on_report_create = (
                request.form.get("slack_webhook_on_report_create") != None
            )
            org.slack_webhook_on_req_create = (
                request.form.get("slack_webhook_on_req_create") != None
            )
            org.slack_webhook_on_req_update = (
                request.form.get("slack_webhook_on_req_update") != None
            )
...
def user_settings():
    if request.method == "POST":
        ...
        if not email:
            ...
            if check_user:
                flash(
                    "Changes not saved. Another user record exists for the email address {}.".format(
                        email
                    )
                )
                ok = False
...
@settings_bp.route("/regenerate_api")
@login_required
...
def integrations():
    if current_user.role == "admin":
        ...
        return render_template(
            "integrations.html", user_details=user_details, page_title="Integrations"
        )
    else:
...
def users_list():
    if current_user.role == "admin":
        users = (
            User.query.filter_by(organization=current_user.organization)
            .order_by(User.name)
            .all()
        )
        return render_template(
            "users.html", users=[u.__dict__ for u in users], page_title="Users"
        )
    else:
        abort(401)  # only admins can access


@settings_bp.route("/user/new")
@settings_bp.route("/user/edit/<user_id>", methods=["GET"])
...
@settings_bp.route("/user/edit/<user_id>")
@login_required
...
@settings_bp.route("/user/delete/<user_id>")
@login_required
...
@settings_bp.route("/users/save/<user_id>", methods=["POST"])
@login_required
    ...
    if current_user.role == "admin":
        ...
        if email is None:
            ...
            if existing_user and (
                user is None or existing_user.id != user.id
...
@settings_bp.route("/change_password")
@login_required
def change_password():
...
@settings_bp.route("/update_integrations", methods=["POST"])
@login_required
