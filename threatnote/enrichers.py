...
def enrich_indicator(data):
    ...
    if ip:
        ...
        update_data.update(
            get_misp_data(indicator, "email-src", misp_api_key, misp_url)
        )
        ...
        if sha256:
            ...
            update_data.update(
                get_misp_data(indicator, "sha256", misp_api_key, misp_url)
            )
        ...
        update_data.update(
            get_misp_data(indicator, "vulnerability", misp_api_key, misp_url)
        )

    if len(update_data) > 0:
        update_data.update(
            {"last_seen": datetime.now(), "last_updated": datetime.now()}
        )
...
def get_vt_file_data(indicator, vt_api_key):
    ...
    if vt_api_key:
        try:
            ...
            data["vt_positives"] = (
                str(response.get("positives")) + "/" + str(response.get("total"))
            )
        except Exception as err:
            print(
                "Virustotal error for indicator{}: {}".format(
                    indicator,
                    traceback.format_exception(type(err), err, err.__traceback__),
                )
            )
...
def get_hunter_data(indicator, hunter_api_key):
    ...
    if hunter_api_key:
        try:
            ...
            print(
                "Hunter error for indicator{}: {}".format(
                    indicator,
                    traceback.format_exception(type(err), err, err.__traceback__),
                )
            )
                ...
                if source.get("source_name") == "mitre-attack":
                    ...
                    if external_id == indicator:
                        if thing.get("x_mitre_permissions_required"):
                            permissions_required = ", ".join(
                                thing.get("x_mitre_permissions_required")
                            )
                        else:
...
def get_whois_data(indicator):
    ...
    try:
        ...
        try:
            data["whois_creationdate"] = result.get("creation_date", {}).strftime(
                "%m/%d/%Y"
            )
        except:
            data["whois_creationdate"] = "Unknown"
        try:
            data["whois_expiration_date"] = result.get("expiration_date", {}).strftime(
                "%m/%d/%Y"
            )
        except:
            data["whois_expiration_date"] = "Unknown"
        try:
            data["whois_last_updated"] = result.get("last_updated", {}).strftime(
                "%m/%d/%Y"
            )
        except:
        ...
        print(
            "Whois error on indicator {} : {}".format(
                indicator, traceback.format_exception(type(err), err, err.__traceback__)
            )
        )
...
def get_urlscan_data(indicator):
    ...
    try:
        ...
        if len(urlscan_result["results"]) > 0:
            ...
            data["urlscan_score"] = str(
                latest_results.get("verdicts", {}).get("overall", {}).get("score")
            )
            data["urlscan_categories"] = str(
                latest_results.get("verdicts", {}).get("overall", {}).get("categories")
            )
            data["urlscan_tags"] = str(
                latest_results.get("verdicts", {}).get("overall", {}).get("tags")
            )
            data["urlscan_malicious"] = str(
                latest_results.get("verdicts", {}).get("overall", {}).get("malicious")
            )
    except Exception as err:
        print(
            "Urlscan error {}".format(
                traceback.format_exception(type(err), err, err.__traceback__)
            )
        )
    return data
...
def get_emailrep_data(indicator, emailrep_api_key):
    data = {
        ...
        "emailrep_suspicious": "",
        "emailrep_references": "",
        "emailrep_blacklisted": "",
        "emailrep_maliciousactivity": "",
        "emailrep_credsleaked": "",
        "emailrep_databreach": "",
        "emailrep_first_seen": "",
        "emailrep_last_seen": "",
        "emailrep_domain_rep": "",
    ...
    if emailrep_api_key:
        try:
            ...
            email_request = requests.get(
                "https://emailrep.io/" + indicator, headers=headers
            )
            ...
            data["emailrep_blacklisted"] = emailrep.get("details", {}).get(
                "blacklisted"
            )
            data["emailrep_maliciousactivity"] = emailrep.get("details", {}).get(
                "malicious_activity"
            )
            data["emailrep_credsleaked"] = emailrep.get("details", {}).get(
                "credentials_leaked"
            )
            ...
            data["emailrep_domain_rep"] = emailrep.get("details", {}).get(
                "domain_reputation"
            )
            data["emailrep_profiles"] = str(emailrep.get("details", {}).get("profiles"))
        except Exception as err:
            print(
                "Emailrep error {}".format(
                    traceback.format_exception(type(err), err, err.__traceback__)
                )
            )
    return data
...
def get_hibp_data(indicator, hibp_api_key):
    ...
    if hibp_api_key:
        ...
        hibp_data = requests.get(
            "https://haveibeenpwned.com/api/v3/breachedaccount/" + indicator,
            headers=headers,
        )
...
def get_riskiq_data(indicator, riskiq_username, riskiq_key):
    ...
    if indicator and riskiq_username and riskiq_key:
        try:
            ...
            risk_info = requests.get(
                "https://api.passivetotal.org/v2/enrichment", params=params, auth=auth
            )
            ...
            risk_osint = requests.get(
                "https://api.passivetotal.org/v2/enrichment/osint",
                params=params,
                auth=auth,
            )
            ...
            print(
                "RiskIQ error for indicator{}: {}".format(
                    indicator,
                    traceback.format_exception(type(err), err, err.__traceback__),
                )
            )
...
def get_otx_data(indicator, ind_type, otx_api_key):
    ...
    if otx_api_key:
        try:
            ...
            if ind_type == "md5":
                indicator_details = otx.get_indicator_details_full(
                    IndicatorTypes.FILE_HASH_MD5, indicator
                )
            elif ind_type == "sha256":
                indicator_details = otx.get_indicator_details_full(
                    IndicatorTypes.FILE_HASH_SHA256, indicator
                )
            elif ind_type == "domain":
                indicator_details = otx.get_indicator_details_full(
                    IndicatorTypes.DOMAIN, indicator
                )
            elif ind_type == "ip":
                indicator_details = otx.get_indicator_details_full(
                    IndicatorTypes.IPv4, indicator
                )
            if indicator_details:
                data["av_general"] = str(indicator_details.get("general", {}))
                data["av_reputation"] = str(
                    indicator_details.get("reputation", {}).get("reputation")
                )
                data["av_malware_data"] = str(
                    indicator_details.get("malware", {}).get("data")
                )
                data["av_url_data"] = str(
                    indicator_details.get("url_list", {}).get("url_list")
                )
                data["av_passive_data"] = str(indicator_details.get("passive_dns"))
                data["av_pulse_count"] = str(
                    indicator_details.get("general", {})
                    .get("pulse_info", {})
                    .get("count")
                )
                if indicator_details.get("analysis", {}).get("analysis"):
                    data["av_tlp"] = str(
                        indicator_details.get("analysis", {})
                        .get("analysis", {})
                        .get("metadata", {})
                        .get("tlp")
                    )
            ...
            print(
                "OTX error for indicator {}: {}".format(
                    indicator,
                    traceback.format_exception(type(err), err, err.__traceback__),
                )
            )
...
def get_shodan_data(indicator, shodan_api_key):
    ...
    if shodan_api_key:
        try:
            ...
            print(
                "Shodan error for indicator{}: {}".format(
                    indicator,
                    traceback.format_exception(type(err), err, err.__traceback__),
                )
            )
...
def get_ipinfo_data(indicator):
    ...
    try:
        ...
        print(
            "IpInfo error for indicator{}: {}".format(
                indicator, traceback.format_exception(type(err), err, err.__traceback__)
            )
        )
        return {}
...
def get_greynoise_data(indicator, greynoise_api_key):
    ...
    if greynoise_api_key:
        try:
            ...
            gn = requests.get(
                "https://api.greynoise.io/v2/noise/context/" + indicator,
                params={},
                headers=headers,
            )
            gn_json = gn.json()
            ...
            print(
                "Greynoise error for indicator{}: {}".format(
                    indicator,
                    traceback.format_exception(type(err), err, err.__traceback__),
                )
            )
...
def get_misp_data(indicator, indicator_type, misp_api_key, misp_url):
    ...
    if misp_api_key:
        try:
            ...
            print(
                "MISP error for indicator{}: {}".format(
                    indicator,
                    traceback.format_exception(type(err), err, err.__traceback__),
                )
            )
...
def export_to_misp(user_details, report, indicators):
    ...
    for indicator in indicators:
        ...
        try:
            misp.add_attribute(
                event_id,
                {"type": indicator_type, "value": indicator_value},
                pythonify=True,
            )
        except:
            pass
