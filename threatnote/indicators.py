...
from threatnote.main import parse_indicators, queue
...
def view_indicators():
    ...
    return render_template(
        "/indicators.html", indicators=indicators, page_title="Indicators"
    )


@indicators_bp.route("/refresh_indicator/<indicator_id>/<report_id>")
@login_required
...
@indicators_bp.route("/edit_indicator/<indicator_id>")
@login_required
def edit_indicator(indicator_id):
    ...
    return render_template(
        ...
        page_title=indicator_details.get("indicator"),
    )


@indicators_bp.route("/update_indicator/<indicator_id>/<report_id>", methods=["POST"])
@login_required
def update_indicator(indicator_id, report_id):
    print("{} {}".format(indicator_id, report_id))
    form_data = request.form
    report_indicator = (
        Links.query.filter_by(report=report_id)
        .filter_by(indicator=indicator_id)
        .first()
    )
    if report_indicator:
        print("link found")
